...
try:
    ...
    file = open("/app/conf/configuration.json", "r")
    conf = json.load(file)
...
db = psycopg2.connect(host=conf["postgres"]["host"],
    ...
    async def run(self, dispatcher: CollectingDispatcher,
        ...
        if (not video_game):
            ...
            if (not rate):
                ...
                dispatcher.utter_template("utter_give_rate", tracker, graphic=rate[0][0], gameplay=rate[0][1],
                                          lifetime=rate[0][2], immersion=rate[0][3], extern=rate[0][4])
        ...
        if (not video_game):
            ...
            if (not requirements[0][5]):
                ...
                if (requirements[0][2]):
                    ...
                    windows_ok = "Non"
                dispatcher.utter_template("utter_give_requirements", tracker, linux_ok=linux_ok, mac_ok=mac_ok,
                                          windows_ok=windows_ok, mac_requirements=requirements[0][3],
    ...
    async def run(self, dispatcher: CollectingDispatcher,
        ...
        if (not video_game):
            ...
            if (not latest_review[0][0]):
                ...
                dispatcher.utter_template("utter_give_latest_review", tracker, latest_review=latest_review[0][0],
...
def fetch_recommandation(user_id):
    cur = db.cursor()
    cur.execute(
        "SELECT name FROM steam_video_games WHERE id = (SELECT game_id FROM of_game_recommandation a WHERE of_user_id = " + user_id + " AND a.game_id NOT IN (SELECT game_id FROM of_game_user_evaluation WHERE of_user_id = " + user_id + ") ORDER BY date_create DESC LIMIT 1)")
    result = cur.fetchall()
    return result[0][0]
...
def fetch_id_game_recommandation(user_id):
    cur = db.cursor()
    cur.execute(
        "SELECT game_id FROM of_game_recommandation a WHERE of_user_id = " + user_id + " AND a.game_id NOT IN (SELECT game_id FROM of_game_user_evaluation WHERE of_user_id = " + user_id + ") ORDER BY date_create DESC LIMIT 1")
    result = cur.fetchall()
    return result[0][0]
...
def save_user_evaluation(game_id, user_id, rate):
    ...
    try:
        cur.execute("INSERT INTO of_game_user_evaluation(of_user_id, game_id, rate, date_create) VALUES(" + str(
            user_id) + "," + str(game_id) + "," + str(rate) + ", '" + str(date_create) + "')")
        db.commit()
    except Exception as e:
        print(e)
...
def fetch_rate(video_game):
    cur = db.cursor()
    cur.execute(
        "SELECT graphic, gameplay, lifetime, immersion, extern FROM of_game_analysis WHERE id_game = (SELECT id FROM steam_video_games WHERE LOWER(name) LIKE LOWER('%" + video_game + "%') LIMIT 1) ORDER BY date_maj DESC LIMIT 1")
    result = cur.fetchall()
    return result
...
def get_latest_game():
    cur = db.cursor()
    cur.execute(
        "SELECT name FROM steam_video_games WHERE id IN (SELECT DISTINCT(id_game) FROM of_game_analysis) order by release_date DESC LIMIT 1")
    result = cur.fetchall()
    return result[0][0]
...
def get_requirements(video_game):
    cur = db.cursor()
    cur.execute(
        "SELECT mac, linux, windows, mac_requirements, linux_requirements, windows_requirements FROM steam_video_games WHERE LOWER(name) LIKE '%" + video_game + "%'")
    result = cur.fetchall()
    return result
...
def get_latest_review(video_game):
    cur = db.cursor()
    cur.execute(
        "SELECT review FROM steam_game_reviews WHERE game_id = (SELECT id FROM steam_video_games WHERE LOWER(name) LIKE '%" + video_game + "%' LIMIT 1) ORDER BY date_create LIMIT 1")
    result = cur.fetchall()
    return result
